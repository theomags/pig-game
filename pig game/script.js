...
const btnSetScore = document.querySelector('.button');

let scores, currentScore, activePlayer, playing, out1;
...
btnSetScore.addEventListener('click', function () {
  if (playing) {
    output1.textContent = input1.value; //getting the value from player
    input1.value = '';
...
btnHold.addEventListener('click', function () {
  if (playing) {
    ...
    if (scores[activePlayer] >= `${output1.textContent}`) {
      // Finish the game
      playing = false;
      diceEl.classList.add('hidden');
